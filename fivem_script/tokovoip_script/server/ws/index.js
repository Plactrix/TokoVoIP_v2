...
(async _ => {
  ...
  if (WSServerIP === undefined) {
    WSServerIP = hostIP;
    console.log(`{yellow AUTOCONFIG:} Setting {cyan WSServerIP} to {cyan ${hostIP}} (you can manually edit in config.js)`);
    await sleep(0);
  }
...
async function masterHeartbeat() {
  axios.post(config.masterServer.heartbeatUrl, {
    tsServer: config.TSServer,
