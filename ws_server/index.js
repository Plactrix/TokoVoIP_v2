...
console.log(
  chalk`Like {cyan TokoVOIP} ? Consider supporting the development: {hex('#f96854') https://patreon.com/Itokoyamato}`
);
...
(async (_) => {
  ...
  config.WSServerPort =
    parseInt(process.env.WSServerPort, 10) || parseInt(config.WSServerPort, 10);
  ...
  if (config.WSServerIP === undefined) {
    ...
    console.log(
      chalk`{yellow AUTOCONFIG:} Setting {cyan WSServerIP} to {cyan ${hostIP}} (you can manually edit in config.js)`
    );
    await sleep(0);
  }
...
{cyan (${wsURI})}`);
    ...
    console.log(
      chalk`Listening on "{cyan ${config.WSServerIP}:${config.WSServerPort}}" (copy and paste this address as "wsServer" in tokovoip_script/c_config.lua)`
    );
...
io.on("connection", async (socket) => {
  ...
  log(
    "log",
    chalk`{${socket.from === "ts3" ? "cyan" : "yellow"} ${
      socket.from
    }} | Connection {cyan opened} - ${socket.safeIp}`
  );
  ...
  if (socket.from === "ts3") {
    ...
    log(
      ...
      chalk`{${socket.from === "ts3" ? "cyan" : "yellow"} ${
        socket.from
      }} | Handshake {green successful} - ${socket.safeIp}`
    );

    socket.on("setTS3Data", (data) => setTS3Data(socket, data));
    socket.on("onTalkStatusChanged", (data) =>
      setTS3Data(socket, { key: "talking", value: data })
    );
    socketHeartbeat(socket);

    // FiveM Handshake
  } else if (socket.from === "fivem" || socket.from === "dispatchclient") {
    socketHeartbeat(socket);
    socket.on("updateClientIP", (data) => {
      if (
        !data ||
        !data.ip ||
        !IPv4Regex.test(data.ip) ||
        !clients[socket.uuid]
      )
        return;
    ...
    socket.on("data", (data) => {
      console.log("SENDING DA DATA BOI");
      onIncomingData(socket, data);
    });
  }
});
...
async function registerHandshake(socket) {
  ...
  while (!client) {
    ...
    client = Object.values(clients).find(
      (item) => !item.fivem.socket && item.ip === socket.clientIp
    );
    try {
...
function onIncomingData(socket, data) {
  const client = clients[socket.uuid];
  if (!socket.uuid || !client || !client.ts3.socket || typeof data !== "object")
    return;
...
async function onSocketDisconnect(socket) {
  log(
    ...
    chalk`{${socket.from === "ts3" ? "cyan" : "yellow"} ${
      socket.from
    }} | Connection {red lost} - ${socket.safeIp}`
  );
  if (socket.from === "fivem" || socket.from === "dispatchclient") {
  ...
  if (socket.uuid && clients[socket.uuid]) {
    ...
    const secondary =
      socket.from === "fivem" || socket.from === "dispatchclient"
        ? "ts3"
        : "fivem";
    if (client[secondary].socket) {
      client[secondary].socket.emit(
        "disconnectMessage",
        `${socket.from}Disconnected`
      );
      if (secondary === "ts3")
        sleep(100) && client[secondary].socket.disconnect(true);
...
async function masterHeartbeat() {
  axios
    .post("https://master.tokovoip.itokoyamato.net/heartbeat", {
      tsServer: config.TSServer,
