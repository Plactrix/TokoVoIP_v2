...
function disconnect(src) {
	...
	if (voip.enableBlockingScreen) {
		document.getElementById("pluginScreen").style.backgroundImage =
			"radial-gradient(rgb(23, 23, 23), rgb(0, 0, 0)";
	}
}
	...
	if (voipStatus !== OK) {
		...
		if (!res.ok)
			...
			if (websocket && websocket.readyState === websocket.OPEN)
				websocket.send(
					`42${JSON.stringify(["updateClientIP", { ip: clientIp }])}`
				);
		}
	}
...
async function init(address, serverId) {
	...
	websocket.onmessage = (evt) => {
		...
		if (msg.event === "setTS3Data") {
			if (msg.data.pluginStatus !== undefined)
				updateScriptData("pluginStatus", parseInt(msg.data.pluginStatus));
			updateWsState("Ts3", OK);
			updateScriptData("pluginVersion", msg.data.pluginVersion);
			updateScriptData("pluginUUID", msg.data.uuid);
			if (msg.data.talking !== undefined)
				$.post(
					`http://${scriptName}/setPlayerTalking`,
					JSON.stringify({ state: msg.data.talking ? 1 : 0 })
				);
		}

		if (msg.event === "ping")
			websocket.send(`42${JSON.stringify(["pong", ""])}`);

		if (msg.event === "disconnectMessage") {
			console.error("disconnectMessage: " + msg.data);
			disconnect("Ts3");
		}
		...
		if (msg.event === "onLatency") {
			...
			document.querySelector(
				"#latency"
			).innerHTML = `Latency Total: ${latency.total}ms<br>Latency FiveM: ${latency.fivem}ms<br>Latency TS3: ${latency.ts3}ms`;
		}
	};
...
function receivedClientCall(event) {
	...
	if (eventName == "updateConfig") {
		...
		if (eventName == "initializeSocket") {
			...
			document.querySelector("#latency").style.display = displayLatency
				? "block"
				: "none";
		}
	}
...
function checkPluginVersion() {
	if (isPluginVersionCorrect()) {
		document.getElementById(
			"pluginVersion"
		).innerHTML = `Plugin version: <font color="green">${voip.pluginVersion}</font> (up-to-date)`;
	} else {
		document.getElementById(
			"pluginVersion"
		).innerHTML = `Plugin version: <font color="red">${voip.pluginVersion}</font> (Required: ${voip.minVersion})`;
...
function isPluginVersionCorrect() {
	...
	if (
		parseInt(voip.pluginVersion.replace(/\./g, "")) <
		parseInt(voip.minVersion.replace(/\./g, ""))
	)
		return false;
	return true;
}
...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		...
		case WRONG_SERVER:
			...
			break;
		case WRONG_CHANNEL:
			msg = `Connected to the wrong TeamSpeak channel, please join the channel: <font color="#01b0f0">${
				(voip.plugin_data.TSChannelWait &&
					voip.plugin_data.TSChannelWait !== "" &&
					voip.plugin_data.TSChannelWait) ||
				voip.plugin_data.TSChannel
			}</font>`;
	...
	document.getElementById(
		"pluginStatus"
	).innerHTML = `Plugin status: <font color="${color}">${
		screenMessage || msg
	}</font>`;
}
...
function updateConfig(payload) {
	...
	document.getElementById(
		"TSServer"
	).innerHTML = `TeamSpeak server: <font color="#01b0f0">${voip.plugin_data.TSServer}</font>`;
	document.getElementById(
		"TSChannel"
	).innerHTML = `TeamSpeak channel: <font color="#01b0f0">${
		voip.plugin_data.TSChannelWait
			? voip.plugin_data.TSChannelWait.replace(/\[[a-z]spacer(.*?)\]/, "")
			: voip.plugin_data.TSChannel.replace(/\[[a-z]spacer(.*?)\]/, "")
	}</font>`;
	document.getElementById(
		"TSDownload"
	).innerHTML = `TS3 Plugin: <font color="#01b0f0">${voip.plugin_data.TSDownload}</font>`;
	document.getElementById(
		"pluginVersion"
	).innerHTML = `Plugin version: <font color="red">Not found</font> (Minimal version: ${voip.minVersion})`;
	document.getElementById("wsInfo").style.display = voip.displayWSInfo
		? "block"
		: "none";
	document.getElementById(
		"wsInfo"
	).innerHTML = `WS Server address: <font color="#01b0f0">${voip.wsServer}</font>`;
}
...
function updateScriptData(key, data) {
	...
	$.post(
		`http://${scriptName}/updatePluginData`,
		JSON.stringify({
			payload: {
				key,
				data,
